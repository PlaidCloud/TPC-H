...
check_variables()
{
	...
	if [ "$count" -eq "0" ]; then
		echo "INSTALL_DIR=\"/greenplum/load\"" >> $MYVAR
		new_variable=$(($new_variable + 1))
	fi
...
#echo "Switching to ADMIN_USER"
#sudo su - $ADMIN_USER

#echo "Setting base directory to repo path"
#cd \"$INSTALL_DIR/$REPO\";

echo "Executing rollout command"
...
sudo -u $ADMIN_USER bash -c "cd \"$INSTALL_DIR/$REPO\"; ./rollout.sh $GEN_DATA_SCALE $EXPLAIN_ANALYZE $RANDOM_DISTRIBUTION $MULTI_USER_COUNT $RUN_COMPILE_TPCH $RUN_GEN_DATA $RUN_INIT $RUN_DDL $RUN_LOAD $RUN_SQL $RUN_SINGLE_USER_REPORT $RUN_MULTI_USER $RUN_MULTI_USER_REPORT $SINGLE_USER_ITERATIONS"
